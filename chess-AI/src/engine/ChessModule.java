...
//rofñ
	...
	//public final int SEARCH_DEPTH = 0;//  4 = 1 ply original S_D=10 i MAX_PLY = 6 i ABWIN = 30
	//public final int MAX_PLY = 0;
	public final int ABWIN = 30;

	//params que provenen de simple engine, i per tant han de ser rebuts, modificats i retornats
	private long nodes;
	private Map hashTable;
	private Position position;
	private int bscore;	//xk es un valor a retornar (obvi)
	private short killers[];
	private boolean computerIsWhite;


	//Akest dos poder els puc posar nomes aqui?
	...
	public String bestMove(String fenPos, int bscore, int search_depth , int max_ply) {
		position =  new Position (fenPos, true);
		...
		if(max_ply==0 && search_depth==0 ){
			// Create an instance of the random class
			Random r = new Random();
			// Get an integer
			int number = r.nextInt();
			int random_number;
			int module=moves.length-1;
			//System.out.println("best move moves length = " + moves.length);
			if (module ==0) random_number=0;
			else random_number = number % module;
			if (random_number < 1) random_number = random_number + module;
			
			//System.out.println("best move random number = " + random_number);
			best = moves[random_number];
			r=null;
		}
		else{
			sortMoves(moves, max_ply);
			for (int i = 0; i < moves.length; i++) {
				...
				lastNodes = nodes;
				int val = -AlphaBeta(search_depth, -beta, -alpha, 0 , max_ply);
				...
				if (val >= beta) {
					...
					beta = INF;
					val = -AlphaBeta(search_depth, -beta, -alpha, 0 , max_ply);
				}
				if (val > score) {
					score = val;
					best = moves[i];
		...
		position = null;
		return (Integer.toString(bscore)+ "," + Short.toString(best) );
	
	}

	private void sortMoves(short a[], int max_ply) {
		...
		for (i = 1; i < a.length; i++) {
			...
			eval[i] = -AlphaBeta(4, -INF, INF, 4 , max_ply);
			position.undoMove();
		}
	...
	private int AlphaBeta(int depth, int alpha, int beta, int howDeep , int max_ply) {
		nodes++;
		...
		for (int i = 0; i < moves.length; i++) {
			...
			if (position.isCheck()) {
				val = -AlphaBeta(depth, -beta, -alpha, howDeep + 1, max_ply);
				...
				val = -AlphaBeta(depth - 2, -beta, -alpha, howDeep + 1, max_ply);
			} else if (position.getLastMove().isCapturing()) {
				val = -AlphaBeta(depth - 2, -beta, -alpha, howDeep + 1, max_ply);		
			} else {
				val = -AlphaBeta(depth - 4, -beta, -alpha, howDeep + 1, max_ply);
			}
